<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            <a href="{{ route('generations.create') }}" class="flex items-center gap-2 px-4 py-2 bg-gray-800 dark:bg-gray-200 border border-transparent rounded-md font-semibold text-xs text-white dark:text-gray-800 uppercase tracking-widest hover:bg-gray-700 dark:hover:bg-white focus:bg-gray-700 dark:focus:bg-white active:bg-gray-900 dark:active:bg-gray-300 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 dark:focus:ring-offset-gray-800 transition ease-in-out duration-150">
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 text-gray-800 dark:text-gray-200">
            ...
            <div class="mt-4 flex flex-col gap-5 bg-white dark:bg-gray-800 shadow-lg sm:rounded-lg p-6">
                        ...
                        @foreach ($generations as $generation)
                            <tr class="border-b border-gray-200 dark:border-gray-700 hover:bg-gray-100 dark:hover:bg-gray-700 transition duration-200">
                                ...
                                <td class="py-4 px-6 flex flex-col gap-2">
                                    ...
                                    <!-- Delete Button -->
                                    <form action="{{ route('generations.destroy', $generation->id) }}" method="POST" class="w-full">
                                        @csrf @method('DELETE')
                                        <button type="submit" class="flex items-center justify-center gap-2 px-4 py-2 w-full bg-red-500 hover:bg-red-600 text-white font-medium rounded-lg transition duration-300"
                                            onclick="return confirm('Are you sure you want to delete this generation?')">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                ...
                @if($generations->isEmpty())
                    <p class="text-center py-6 text-gray-500 dark:text-gray-400">No generations found.</p>
                @endif
{{ $generations->links("pagination::tailwind") }}
            </div>
        </div>
    </div>
</x-app-layout>
