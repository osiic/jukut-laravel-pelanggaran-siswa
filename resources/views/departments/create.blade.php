<x-app-layout>
    <x-slot name="header">
        ...
        <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">
            {{ __('Create a new department by entering its name.') }}
        </p>
    </x-slot>

    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <form action="{{ route('departments.store') }}" method="POST" class="bg-white dark:bg-gray-800 p-6 shadow sm:rounded-lg mt-6 space-y-6">
                @csrf
                ...
                <div class="flex items-center gap-4">
                    ...
                    @if (session('status') === 'department-added')
                        <p
                            x-data="{ show: true }"
                            x-show="show"
                            x-transition
                        ...
                        >{{ __('Department successfully added.') }}</p>
                    @endif
                </div>
            </form>
        </div>
